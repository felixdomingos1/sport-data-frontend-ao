...
import { ArrowLeft, Trophy, Calendar, Tag, Layers, Shield, BarChart3 } from 'lucide-react';
...
const BracketView: React.FC<{ confrontos: Partida[] }> = ({ confrontos }) => {
  const grouped = confrontos.reduce<Record<number, Partida[]>>((acc, p) => {
    const round = p.rodada ?? 1;
      ...
      <div className="flex gap-6 min-w-max px-4">
        {sortedRounds.map((round) => (
          <div key={round} className="flex flex-col gap-4 min-w-[280px]">
            <h4 className="text-white/40 text-[10px] font-bold uppercase tracking-widest text-center mb-1">
              Rodada {round}
            </h4>
            {grouped[round].map((partida) => {
              const participantes = partida.jogadores ?? [];
              const teamA = participantes[0]?.participacao ?? participantes[0];
              const teamB = participantes[1]?.participacao ?? participantes[1];

              return (
                <div
                  key={partida.id}
                  className="bg-white/[0.03] border border-white/5 rounded-lg overflow-hidden hover:border-white/10 transition-colors"
                >
                  ...
                  <div className="divide-y divide-white/[0.03]">
                    {participantes.length > 0 ? (
                        ...
                        >
                          <div className="flex items-center gap-2 min-w-0">
                            ...
                            <span className={`text-xs font-semibold truncate ${jp.vencedor ? 'text-white' : 'text-white/60'}`}>
                              {jp.participacao?.nomeEquipa ?? jp.participacao?.nomeAtleta ?? `Participante ${idx + 1}`}
                            </span>
                          </div>
                      ...
                      <div className="px-3 py-2.5 text-white/30 text-xs text-center">
                        TBA
                      </div>
                    )}
                  </div>
                  <div className="px-3 py-1 bg-white/[0.02]">
                    <span className="text-white/30 text-[10px]">
                      {formatMatchDate(partida.dataHora)}
                    </span>
                  </div>
                </div>
              );
...
const ChaveamentoDetalhe: React.FC = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      competicaoService.getConfrontos(activeTab).catch(() => []),
      competicaoService.getClassificacao(activeTab).catch(() => []),
    ])
      .then(([confs, classif]) => {
        setConfrontos(confs);
  ...
  const showBracket = activeFase && (activeFase.tipo === 'ELIMINATORIA');
  const showStandings = !showBracket;
    ...
    <div className="min-h-screen bg-[#0A0A0B] text-white">
      ...
      <div className="max-w-5xl mx-auto px-6 py-8 space-y-8">
        ...
        {fases.length > 0 && (
          ...
          >
            <h2 className="text-sm font-bold text-white/40 uppercase tracking-widest mb-4">Fases</h2>
            <div className="flex gap-2 flex-wrap">
              {fases.sort((a, b) => a.ordem - b.ordem).map(fase => (
                <button
                  key={fase.id}
                  onClick={() => setActiveTab(fase.id)}
                  className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-semibold tracking-wide transition-all duration-200 ${activeTab === fase.id
          ...
          >
            <div className="px-5 py-3 border-b border-white/5 flex items-center gap-3">
              ...
              <span className="text-white/30 text-[10px] uppercase tracking-widest font-semibold">
                {activeFase.tipo === 'ELIMINATORIA' ? 'Chaveamento' : activeFase.tipo === 'GRUPOS' ? 'Classificação' : 'Misto'}
              </span>
            </div>
            ...
            {loadingPhase ? (
              ...
              <div className="p-2">
                {showBracket && <BracketView confrontos={confrontos} />}
                {showStandings && classificacao.length > 0 && <StandingsTable data={classificacao} />}
                {showStandings && classificacao.length === 0 && !showBracket && (
                  <div className="py-12 text-center text-white/30">
                    <p className="text-sm">Sem classificação disponível</p>
                  </div>
                )}
                {showBracket && confrontos.length === 0 && (
                  <div className="py-12 text-center text-white/30">
                    <p className="text-sm">Sem confrontos disponíveis</p>
                  </div>
                )}
              </div>
            )}
