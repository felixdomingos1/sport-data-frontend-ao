    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-[#0f0f0f] rounded-2xl p-5 border border-[#1a1a1a]">
          ...
          <p className="text-3xl font-bold text-white">{proximos.length}</p>
          <p className="text-xs text-gray-500 mt-1">Próximas competições</p>
        </div>
        <div className="bg-[#0f0f0f] rounded-2xl p-5 border border-[#1a1a1a]">
          <Trophy className="w-4 h-4 text-gray-500 mb-3" />
          <p className="text-3xl font-bold text-white">{metricas?.totalCompeticoes ?? 0}</p>
          <p className="text-xs text-gray-500 mt-1">Total participações</p>
        </div>
        <div className="bg-[#0f0f0f] rounded-2xl p-5 border border-[#1a1a1a]">
          <Star className="w-4 h-4 text-gray-500 mb-3" />
          <p className="text-3xl font-bold text-white">{metricas?.inscricoesAtivas ?? 0}</p>
          <p className="text-xs text-gray-500 mt-1">Inscrições activas</p>
        </div>
      ...
      <div className="bg-[#0f0f0f] rounded-2xl border border-[#1a1a1a] overflow-hidden">
        ...
        <div className="divide-y divide-[#1a1a1a]">
          {proximos.length === 0 ? (
            <p className="px-5 py-10 text-sm text-gray-500 text-center">Nenhuma competição activa disponível.</p>
          ) : (
            proximos.map((comp) => (
              <div key={comp.id} className="flex items-center gap-4 px-5 py-4 hover:bg-[#141414]">
                ...
                <button className="p-2 rounded-lg text-gray-400 hover:text-white hover:bg-[#2a2a2a]">
      ...
      <div className="bg-[#0f0f0f] rounded-2xl border border-[#1a1a1a] overflow-hidden">
        ...
        <div className="divide-y divide-[#1a1a1a]">
          {historicoFiltrado.length === 0 ? (
            <p className="px-5 py-12 text-sm text-gray-500 text-center">Nenhuma participação registada.</p>
          ) : (
            historicoFiltrado.map((item) => (
              <div key={item.id} className="px-5 py-4 hover:bg-[#141414]">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  <div>
                    <p className="text-sm font-medium text-white">{item.campeonato?.nome ?? 'Competição'}</p>
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-xs text-gray-500">{item.vitorias}V / {item.derrotas}D / {item.empates}E</p>
                  </div>
                </div>
              </div>
            ))
