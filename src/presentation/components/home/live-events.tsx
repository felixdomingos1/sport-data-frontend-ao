...
const LiveEvents: React.FC = () => {
  ...
  const getStatusConfig = (status: Event['status']) => {
    const configs = {
      live: {
        label: 'AO VIVO',
        className: 'bg-linear-to-r from-red-600 to-red-500 text-white shadow-lg shadow-red-500/30',
        icon: <Activity className="w-3 h-3 animate-pulse" />,
        animated: true,
      },
      halftime: {
        label: 'INTERVALO',
        className: 'bg-linear-to-r from-yellow-600 to-orange-500 text-white',
        icon: <Clock className="w-3 h-3" />,
        animated: false,
      },
      upcoming: {
        label: 'PRÓXIMO',
        className: 'bg-linear-to-r from-green-600 to-emerald-500 text-white',
        icon: <Calendar className="w-3 h-3" />,
        animated: false,
      },
      finished: {
        label: 'FINALIZADO',
        className: 'bg-linear-to-r from-gray-600 to-gray-500 text-white',
    ...
    <section className="py-20 bg-linear-to-br from-slate-50 via-white to-slate-100 dark:from-gray-900 dark:via-gray-950 dark:to-black">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-end mb-12 gap-6">
          <div>
            <div className="flex items-center gap-3 mb-3">
              <div className="w-1 h-8 bg-linear-to-b from-red-600 to-yellow-500 rounded-full"></div>
              <h2 className="text-4xl lg:text-5xl font-black bg-linear-to-r from-gray-900 to-gray-700 dark:from-white dark:to-gray-300 bg-clip-text text-transparent">
                Eventos Desportivos
              </h2>
            </div>
          ...
          <div className="flex gap-4">
            <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-white dark:bg-gray-800 shadow-md">
              ...
              <span className="text-sm font-medium">{events.filter(e => e.status === 'live').length} AO VIVO</span>
            </div>
            {hasMore && (
              <button
                onClick={() => setVisibleCount(p => p + 3)}
                className="flex items-center gap-2 px-6 py-2 rounded-full bg-linear-to-r from-red-600 to-red-500 text-white font-semibold hover:from-red-700 hover:to-red-600 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105"
              >
                Ver mais
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8">
          {displayedEvents.map((event, index) => {
            ...
            return (
              ...
              >
                <div className="relative bg-white dark:bg-gray-900 rounded-2xl overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-300">
                  ...
                  <div className="p-5">
                    ...
                    <div className="mb-5">
                      <div className="flex items-center justify-between mb-4">
                        ...
                        <div className="flex-1 text-center">
                          ...
                          <p className="font-bold text-gray-800 dark:text-white text-sm line-clamp-2">
                            {event.homeTeam.name}
                          </p>
                        </div>

                        {/* PLACAR CENTRAL */}
                        <div className="mx-4 px-4 py-2 rounded-xl bg-gray-100 dark:bg-gray-800">
                    ...
                    <div className="flex items-center justify-between gap-3 pt-3 border-t border-gray-100 dark:border-gray-800">
                      <Link
                        to={`/eventos/${event.id}`}
                        className="flex-1 flex items-center justify-center gap-2 px-4 py-2 bg-linear-to-r from-red-600 to-red-500 text-white rounded-xl font-medium hover:from-red-700 hover:to-red-600 transition-all duration-300 shadow-md hover:shadow-lg"
                      >
                        <span>Detalhes</span>
                        <ChevronRight size={16} />
                      </Link>

                      <button className="p-2 rounded-xl bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-red-100 dark:hover:bg-red-900/30 hover:text-red-600 transition-colors">
                        <Heart size={18} />
                      </button>
        ...
        <motion.div
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          className="mt-16 bg-linear-to-r from-red-600 via-red-500 to-yellow-500 rounded-2xl overflow-hidden shadow-2xl"
        >
          <div className="px-8 py-6 text-white">
            <div className="flex flex-col md:flex-row justify-between items-center gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-2xl font-bold">Próximos Eventos Importantes</h3>
                  <p className="text-white/90">Não perca as principais competições do calendário desportivo angolano</p>
                </div>
              </div>
              <Link
                to="/eventos"
                className="px-6 py-2 bg-white text-red-600 rounded-full font-semibold hover:shadow-lg transition-shadow whitespace-nowrap"
              >
